...
【回答のルール】
- ドキュメントの内容に基づいて回答する
- 意味的に同じ内容を指している場合は、ドキュメントに基づいて柔軟に回答を生成する
- ドキュメントの内容に基づいた回答をするとき、その回答の情報の元になったドキュメントを明示する「（基づいた資料の名前）によると〜」のようにする
- 複数のドキュメントを参照する場合は「（基づいた資料の名前）によると〜。また（基づいた資料の名前）では〜」のように区別する
- ドキュメントに情報がない場合は「申し訳ありませんが、ご提供いただいた資料にはその情報が見当たりませんでした」と答える
- 分かりやすく、自然な日本語で回答する
- ユーザーの質問の意図を理解し、適切に回答する
"""
                },
                {
                    "role": "user",
